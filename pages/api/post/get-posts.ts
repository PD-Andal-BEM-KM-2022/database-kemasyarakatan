...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  const maxPage = Math.ceil((await collection.countDocuments()) / Number(limit));
  ...
  if(page >= posts.length) return res.status(200).json({ message: "Page not valid" });
  ...
  if (limit > posts.length)
    ...
    return res.status(200).json({maxPage, posts: posts.slice(page, page + Number(limit))});
